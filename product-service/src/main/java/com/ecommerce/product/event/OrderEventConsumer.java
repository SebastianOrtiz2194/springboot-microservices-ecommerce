 ...
 * Uses a deduplication table and atomic DB updates to ensure idempotent and
 * race-free processing (prevents oversell).
 */
@Component
public class OrderEventConsumer {
     ...
     * Uses atomic {@code UPDATE ... WHERE stock >= qty} to prevent oversell and
     * optimistic locking via {@code @Version}. Evicts Redis caches to avoid stale stock.
    ...
    public void handleOrderPlaced(OrderPlacedEvent event) {
        ...
        for (OrderItemEvent item : event.items()) {
            if (item.quantity() <= 0) {
                log.warn("invalid_quantity order_id={} product_id={} qty={}", event.orderId(), item.productId(), item.quantity());
                continue;
            }
            ...
            if (updated == 1) {
                log.info("stock_decremented product_id={} qty={}", item.productId(), item.quantity());
                continue;
            }
            ...
            if (current < item.quantity()) {
                log.warn("insufficient_stock order_id={} product_id={} current={} requested={}",
                        event.orderId(), item.productId(), current, item.quantity());
            ...
            try {
                ...
                if (locked != null) {
                    int lockedStock = locked.getStockQuantity() != null ? locked.getStockQuantity() : 0;
                    if (lockedStock >= item.quantity()) {
                        locked.setStockQuantity(lockedStock - item.quantity());
                        productRepository.save(locked);
                        log.info("stock_updated_via_lock product_id={} new_stock={}", item.productId(), locked.getStockQuantity());
                    } else {
                        log.warn("insufficient_stock_after_lock order_id={} product_id={} current={} requested={}",
                                event.orderId(), item.productId(), lockedStock, item.quantity());
                    }
                }
            } catch (Exception ex) {
                log.error("stock_update_failed product_id={} order_id={}", item.productId(), event.orderId(), ex);
                throw new InsufficientStockException("Failed to update stock for product " + item.productId() + ": " + ex.getMessage());
            }
        }
