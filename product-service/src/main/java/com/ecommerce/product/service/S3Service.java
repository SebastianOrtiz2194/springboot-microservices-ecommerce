 ...
 * <p>
 * Stores the S3 <b>key</b> in the database; pre-signed URLs are generated on read
 * to avoid expiry. Validates file size, MIME type, and filename to prevent
...
public class S3Service {
    ...
    /**
     * Uploads a file to the configured S3 bucket and returns the stored S3 key.
     * The pre-signed URL should be generated via {@link #createPresignedUrl(String)} on read.
     *
    ...
    public String uploadImage(MultipartFile file) throws IOException {
        ...
        log.info("image_uploaded bucket={} key={} contentType={} size={}", bucket, key, contentType, file.getSize());
        return key;
    }
    ...
    private void validateFile(MultipartFile file) {
        ...
        if (file.getSize() > maxFileSize) {
            throw new IllegalArgumentException("File too large: " + file.getSize() + " bytes, max " + maxFileSize);
        }
        String contentType = file.getContentType();
        if (contentType == null || !ALLOWED_CONTENT_TYPES.contains(contentType.toLowerCase())) {
            throw new IllegalArgumentException("Unsupported content type: " + contentType + ". Allowed: " + ALLOWED_CONTENT_TYPES);
        }
        if ("dev-bucket-placeholder".equals(bucket) || "your-bucket-name".equals(bucket)) {
            log.warn("s3_bucket_placeholder bucket={} — configure S3_BUCKET_NAME for real uploads", bucket);
        }
    }
